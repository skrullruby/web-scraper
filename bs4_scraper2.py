...
for URL in pagesToScrape:
	...
	webpage = urllib.request.urlopen(URL)			# fetch webpage
	...
	for prop in properties:
		...
		state = infos[0].text.strip() + infos[1].text.strip()
		amount = infos[2].text.strip()
		acres = infos[3].text.strip()
		ref = infos[4].text.strip()
...
for prop in property_arr:
	csvwriter.writerow(prop)		# write a row in the file
